...
![storage_4](../assets/storage_4.jpg) 
...
上表中，docker daemon使用的是aufs存储驱动，docker宿主机使用的是extfs。docker daemon选择何种存储驱动器，一部分取决于其宿主机，因为有些后台文件系统支持docker daemon使用不同的类型的存储驱动器，而有些不是。如表所示：
